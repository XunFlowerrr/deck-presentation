...
import { DISTANCE, DURATION, fadeInUp, stagger } from "../../lib/motion.ts";
import { hydeDensityGapHeader } from "../../content/slides/system-slides";
  ...
  return (
    ...
    >
      ...
      <motion.g
        ...
        style={{ transformOrigin: "89px 61px" }}
      >
        <rect
          x="68"
          y="54"
    ...
    (async () => {
      while (alive) {
        ...
        await dotCtrl.start({
          scale: 1,
          opacity: 1,
          transition: { duration: 0.4, type: "spring", bounce: 0.55 },
        });
        if (!alive) break;
...
const CONCEPTS = [
  {
    ...
    enBody:
      "User queries are usually short and low-context, unlike enterprise documents which are dense with information. This mismatch makes conventional retrieval produce low similarity scores and bring back irrelevant results.",
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <ThaiText en={concept.enBody}>{concept.body}</ThaiText>
        </p>
      </div>
...
export function HydeDensityGap() {
  return (
    <SlideShell glows={GLOWS}>
      <SlideHeader
        label={hydeDensityGapHeader.label}
        title={hydeDensityGapHeader.title}
        highlight={hydeDensityGapHeader.highlight}
        tagline={hydeDensityGapHeader.tagline}
        marginBottom={20}
      />
